...
gotoxy(5,16);
printf("Press first character for further operations(P,C,A,M) ");
ch=toupper(getch());
...
default:gotoxy(5,18);
printf("Please Enter right character(P,C,A,M)");
getch();
}
}
while(ch!='M');
...
printf("Welcome to Medical Store ");
 ...
 default:
 printf("Please enter right character (M,E)");
 getch();
 }
 }
 while(ch!='E');
...
gotoxy(7,10);
printf("Id. Medicine name.   Qty.   Rate.   Total ");
gotoxy(7,12);
j=14;
while(fscanf(ptrp_r,"%s   %s   %d    %f    %f\n",p_r.medi_id,p_r.medir_name,&p_r.qty,&p_r.rate,&p_r.total)!=EOF)
{
gotoxy(6,j);
